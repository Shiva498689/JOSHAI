...
# CHANGED: Replaced local host dictionary with your live Render Connection String
...
async def init_db_pool():
    global pool
    # CHANGED: Connecting the pool via the URL directly
    pool = await asyncpg.create_pool(RENDER_DB_URL, min_size=1, max_size=10)
...
async def update_user(conn: asyncpg.Connection, email: str, update_data: upadate_schema) -> Optional[response_schema]:
    ...
    if update_data.preferred_branches is not None:
        ...
        params.append(json.dumps(update_data.preferred_branches))
        idx += 1
    ...
    if not sets:
        return await get_by_email(conn, email)

    sets.append("updated_at = NOW()")
    
    # FIX: Assigned user_id to the dynamic parameter index ($idx) instead of stacking variables incorrectly
    query = f"""
        UPDATE users
        SET {", ".join(sets)}
        WHERE id = ${idx}
